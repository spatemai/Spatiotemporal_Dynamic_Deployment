...
#(agent_n,3)
agent_ini=np.load('data\\best_action_ini_final.npy')[0]
...
#(size,2)
node_ini=np.load('data\\best_node_ini_final.npy')[0]
...
def step(a):
    ...
    for i in range(user_n):
        ...
        renwu[np.argmin(distance_1[i])]=node_data[np.argmin(distance_1[i])]+user_data[t,i]
    ...
    s_ = np.concatenate((user[t+1].flatten() / 50 - 1, (user_data[t+1].flatten() - 100000 * 8) / (900000 * 8) * 2 - 1,agent_nor.flatten(),node_nor.flatten()))
